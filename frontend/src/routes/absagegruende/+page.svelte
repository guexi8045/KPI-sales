<script lang="ts">
  ...
  interface AbsageData {
    month: string;
    category_name: string;
    num_deals: number;
  }

  let rawData: AbsageData[] = [];
  ...
  const fetchData = async () => {
    const res = await fetch('http://127.0.0.1:5000/api/absagegruende');
  ...
  function updateChartData() {
    ...
    const datasets: ChartDataset<'line'>[] = shownCategories.map((cat) => {
      const data = months.map((month) => {
        ...
        return record ? record.num_deals : 0;
      });
    ...
    if (showTotal) {
      const totalData = months.map(month => {
        return allCategories.reduce((sum, cat) => {
          const record = rawData.find((d) => d.month === month && d.category_name === cat);
          return sum + (record ? record.num_deals : 0);
        }, 0);
      });
  ...
  function exportPDF() {
    html2canvas(chartContainer).then((canvas) => {
      ...
      pdf.setFontSize(12);
      pdf.text('Absagegründe pro Monat', 14, 14);
      pdf.addImage(imgData, 'PNG', 10, 20, 270, 160);
      pdf.save('absagegruende_report.pdf');
    });
  }
  ...
  function exportCSV() {
      ...
      shownCats.forEach(cat => {
        if (cat === TOTAL_LABEL) {
          const total = allCategories.reduce((sum, c) => {
            ...
            return sum + (r ? r.num_deals : 0);
          }, 0);
          row.push(total.toString());
        } else {
          const record = rawData.find((d) => d.month === month && d.category_name === cat);
          row.push(record ? record.num_deals.toString() : '0');
        }
      });
    ...
    a.href = url;
    a.download = 'absagegruende_daten.csv';
    a.click();
