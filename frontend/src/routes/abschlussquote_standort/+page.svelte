<script lang="ts">
  ...
  const endpoint = 'http://127.0.0.1:5000/api/abschlussquote_standort_roh';
...
<h2>Abschlussquote pro Besichtigungsstandort</h2>
...
<!-- Eigene Analyse unten -->
<div style="margin-top: 2rem;">
  <h3>Eigene Analyse</h3>
  ...
  <p>
    Abschlussquote für <strong>{selectedCategory}</strong>
    im Zeitraum <strong>{startMonth}</strong> – <strong>{endMonth}</strong>:
    <strong>{customQuote}</strong>
  </p>
</div>
